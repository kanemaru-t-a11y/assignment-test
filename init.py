...
def init_session_id(): # セッションIDの初期化
    ...
    if logger: # ロガーが存在する場合
        logger.info(f"Session ID initialized: {st.session_state.session_id}") # ログ出力
...
def init_session_state(): # セッションステートの初期化
    ...
    st.session_state.setdefault("flow_mode", None) # "faculty" | "department" | "research" | "campus"
...
def init_retrievers(): # ベクトルDB初期化
    ...
    if logger: # ログ出力
        logger.debug(f"recursive_file_check結果: {len(docs_all)}件のドキュメントを取得") # ログ出力
    ...
    labels = [r"学部名[:：]", r"学科名[:：]", r"施設名[:：]", r"イベント名[:：]", r"証明書名[:：]"] # 分割ラベル
    ...
    for doc in docs_all: # ドキュメント分割と仕分け
        chunks = [] # 分割チャンク
        matches = re.findall(split_pattern, doc.page_content, flags=re.DOTALL) # パターンマッチ
    ...
    dbs = {} # ベクトルDB辞書
    for name in ["all", "faculty", "department", "research", "campus"]: # Chromaに5コレクション作成（collection_name別）
    ...
    def _make_kwargs(base_kwargs: dict, contains_key: str | None): # フィルタ付き検索用kwargs作成
        ...
        kw["filter"] = {"source": {"$contains": contains_key}} # source に特定フォルダ名が含まれているものだけ返す
        return kw # 返す

    base = {"k": cf.TOP_K, "fetch_k": 20, "lambda_mult": 0.5} # ベースkwargs
    retrievers = {
...
def load_data_sources(): # データソース読み込み
    ...
    if getattr(cf, "USE_WEB_SOURCES", False): # Web取り込み（robots.txt 準拠、許可URLのみ）
        web_docs = load_web_sources_safe(getattr(cf, "WEB_URLS", [])) # Webドキュメント取得
...
def _get_robots_parser(base_url): # robots.txt パーサ取得
    ...
    robots_url = f"{parts.scheme}://{parts.netloc}/robots.txt" # robots.txt URL
...
def load_web_sources_safe(urls): # Webソース読み込み
    ...
    for url in urls: # URL走査
        ...
        try: # 読み込み
            ...
            docs.extend(loaded)
            if logger: # ログ出力
                logger.info(f"[web] Loaded {len(loaded)} docs from: {url}") # ログ出力
            time.sleep(1.0)  # 負荷配慮
        except Exception as e: # 読み込み失敗
            if logger: # ログ出力
